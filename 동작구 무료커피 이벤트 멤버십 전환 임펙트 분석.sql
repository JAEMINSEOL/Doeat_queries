...
-- , isnull(g.get_promo, '미발급') ||'-'|| isnull(p.use_promo, '미사용') as 이벤트_커피_사용
, isnull(p.use_promo, '미사용') as 이벤트_커피_사용
, isnull(a_status, '미가입자')  as 이벤트_전
, isnull(b_status, '미가입자') as 이벤트_후
, 이벤트_전 ||'-'|| 이벤트_후 as 이벤트_전_후_May_16_19
, isnull(c_status, '미가입자') as 이주_뒤_상태_June_3
...
--             where log_type = 'FreePBCoffeeDongjakOpenAlert' and log_action = '이벤트 오픈 바텀싯 열림'  and date(created_at) >= '2025-05-17'
