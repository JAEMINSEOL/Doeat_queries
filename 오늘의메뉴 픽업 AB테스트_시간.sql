...
, s_order as( select date_trunc('hour',o.created_at) as time
                ...
                where time >= '2025-10-15 14:00'
                group by 1,2,3,4
                order by time desc
                )
select *
        , sum(ord_cnt_A) over (partition by date order by hour_24 rows between unbounded preceding and current row) as cumul_ord_cnt_A
        , sum(ord_cnt_B_delivery) over (partition by date order by hour_24 rows between unbounded preceding and current row) as cumul_ord_cnt_B_delivery
        , sum(ord_cnt_B_pickup) over (partition by date order by hour_24 rows between unbounded preceding and current row) as cumul_ord_cnt_B_pickup
        from s_order
