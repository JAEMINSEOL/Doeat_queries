select tor.rider_id,
       r.rider_name,
       o.store_id,
       case o.store_id when 6909 then '관악구' when 6919 then '동작구' else null end as 지역,
        '{{메뉴명}}' as 메뉴명,
        count(case when i.product_id =  ({{관악메뉴ID}}) or i.product_id = ({{동작메뉴ID}}) then 1 else 0 end) over (partition by tor.rider_id order by o.delivered_at) as 픽업수량,
        o.delivered_at as delv_time
        , tor.created_at as on_time
    --   t.id,
    --     tor.team_order_id,
    --     tor.created_at as team_order_rider_created_at,
    --     o.status

    --   sum(case when i.product_id =  ({{관악메뉴ID}}) or i.product_id = ({{동작메뉴ID}}) then 1 else 0 end) as "픽업 수량"
       --  
from orders o
...
where 1=1
    and ((i.product_id in ({{관악메뉴ID}})) or ( i.product_id in  ({{동작메뉴ID}}))) 
    and o.created_at between date_sub(str_to_date(CONCAT('{{날짜}}', ' ', '{{타임}}'), '%Y-%m-%d %H:%i:%s'), INTERVAL 1 minute)
                    AND date_add(str_to_date(CONCAT('{{날짜}}', ' ', '{{타임}}'), '%Y-%m-%d %H:%i:%s'), INTERVAL 60 minute)
...
-- group by 1,2,3,4,5
-- and 지역 = '{{시군구}}'
order by 픽업수량 desc, delv_time desc
