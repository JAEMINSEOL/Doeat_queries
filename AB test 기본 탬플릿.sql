with 
p_order as(
    select o.user_id, o.created_at, o.type, o.order_price
    from doeat_delivery_production.orders o
                ...
                and not (extract (hour from o.created_at) < 5 and extract (hour from o.created_at) > 1)
)
...
select
    ...
    , count(case when has_main = 1 and has_o=1 then 1 end) as 전체_결제
        , count(case when has_main = 1 and has_ot=1 then 1 end) as 일반두잇_결제
...
from
    (select
        ...
        , max(distinct case when ot.user_id is not null then 1 else 0 end) as has_ot
        , avg(aov) as aov
         from p_log l 
         ...
         left join (select 
                distinct user_id, o.created_at::date as date
               from p_order o) o on o.user_id = l.user_id and o.date = l.created_at::date
         left join (select
                user_id, o.created_at::date as date, avg(order_price) as aov
               from p_order o
               where type = 'TEAMORDER'
               group by 1,2) ot on ot.user_id = l.user_id and ot.date = l.created_at::date
               
         left join (select
                distinct user_id, o.created_at::date as date
               from p_order o
               where type like '%SEVEN%' or type like '%119%') oc on oc.user_id = l.user_id and oc.date = l.created_at::date
         left join (select
                distinct user_id, o.created_at::date as date
               from p_order o
               where type = 'CURATION_PB') oa on oa.user_id = l.user_id and oa.date = l.created_at::date
