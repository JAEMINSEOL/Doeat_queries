select case when h.sector_id in(1,3,4) then '비실험섹터' when h.sector_id in(2) then '실험섹터' end as sector_type
    ...
    , count(distinct case when date(o.created_at) = '2025-07-02' then o.id end) as order_cnt_0702
    , count(distinct case when date(o.created_at) = '2025-06-25' then o.id end) as order_cnt_0625
    , (order_cnt_0702-order_cnt_0625)*100.0/nullif(order_cnt_0625,0) as order_cnt_change_rate
...
where o.orderyn = 1
  ...
  and (o.created_at between '2025-07-02 10:00:00' and  '2025-07-02 17:00:00'
        or o.created_at between '2025-06-25 10:00:00' and  '2025-06-25 17:00:00')
group by 1,2
