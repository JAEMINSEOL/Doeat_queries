select case when h.sector_id in(1,3,4) then '비실험섹터' when h.sector_id in(2) then '실험섹터' end as sector_type
    ...
    , count(distinct case when date(o.created_at) = '2025-07-01' then o.id end) as order_cnt_0702
    , count(distinct case when date(o.created_at) = '2025-06-24' then o.id end) as order_cnt_0624
    , (order_cnt_0702-order_cnt_0624)*100.0/nullif(order_cnt_0624,0) as order_cnt_change_rate
...
where o.orderyn = 1
  ...
  and (o.created_at between '2025-07-01 17:45:00' and  '2025-07-01 20:45:00' or
        o.created_at between '2025-06-24 17:45:00' and  '2025-06-24 20:45:00')
group by 1,2
