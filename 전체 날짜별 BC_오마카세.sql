...
select distinct dt as date,
       extract(hour from u.created_at::timestamp) as hour,
       user_id
from service_log.user_log u
where dt = '{{날짜}}'
    and u.created_at::timestamp between '{{날짜}}' || ' 19:58:00' and dateadd(day,1,'{{날짜}}') || ' 00:50:00' 
)
,
-- 분자 : 구매 유저
order_user as (
...
where o.created_at::timestamp between '{{날짜}}' || ' 19:58:00' and dateadd(day,1,'{{날짜}}') || ' 00:50:00' 
