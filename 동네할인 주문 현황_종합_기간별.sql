...
,period_set as(
select date_trunc('month', created_at) as start_date
        , 'month' as period
        , coalesce(menu_name,'#_전체') as menu
        , sum(order_price) as 매출총액
from p_order
-- where date(created_at) >= '2025-08-08'
group by start_date, rollup(menu_name)

union all

select date_trunc('week', created_at) as start_date
        , 'week' as period
        , coalesce(menu_name,'#_전체') as menu
        , sum(order_price) as 매출총액
from p_order
...
select date_trunc('week', case when mod(extract(week from created_at),2)=0 then created_at else dateadd('week',-1,created_at) end) as start_date
        , 'biweek' as period
        , coalesce(menu_name,'#_전체') as menu
        , sum(order_price) as 매출총액
from p_order
...
select * from period_set
where period='{{period}}'
order by start_date, menu
