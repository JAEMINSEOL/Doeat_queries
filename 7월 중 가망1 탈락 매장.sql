...
from (select * 
        from(select *, row_number() over (partition by product_id order by target_date desc) as rn
              ...
              where target_date between '2025-07-01' and '2025-07-31'
                and store_type = 'NO_EXCELLENT_PRODUCT'
                and sigungu in ({{지역}})
...
join (select id as store_id, phone_number,president_phone_number from doeat_delivery_production.store s) s on s1.store_id = s.store_id
join (select id as product_id, menu_name from doeat_delivery_production.doeat_777_product p) p on p.product_id=s1.product_id
where 1
