...
select 
    ...
    q.feedback_count,
    q.like_ratio,
    mp.onepeak_yn, mp.store_product_type, mp.prev_store_product_type ,mp.next_store_product_type ,
    coalesce(mp.excellent_product_count,0) as excellent_product_count

from doeat_delivery_production.curation_product_qualification_realtime_log q 
...
left join (
            select distinct product_id, store_id, shop_in_shop_id, excellent_product_count,target_date,store_product_type,prev_store_product_type,next_store_product_type,onepeak_yn
            from doeat_data_mart.mart_store_product_daily
...
and q.target_date = '{{target_date}}'
and mp.onepeak_yn='NORMAL'
order by sigungu,q.target_date desc, after_qualification,p.product_type,p.store_id,q.product_id
