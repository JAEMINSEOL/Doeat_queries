...
opt_base as(
            select s.id as store_id,
                            ...
                            join (select * from(
                                select price, option_category_id, id, is_sold_out, last_modified_at, option_name,
                                        min(op) over (partition by id) as op_last
                                from history_schema_v2.menu_option
                                where last_modified_at::date >='2025-05-01' and is_deleted=0 and is_hidden=0
                                ) where op_last!='D'
 ...
 select distinct slot_time,sigungu
                    , last_timestamp, product_id, menu_name, option_id, option_name,is_sold_out,option_category_id, category_name

                from( SELECT *
                    FROM (SELECT
                            t.slot_time,d.product_id, menu_id, option_id, d.sigungu, is_sold_out, last_timestamp,menu_name, option_name,option_category_id, category_name,
                            ROW_NUMBER() OVER (
...
where slot_time between '2025-08-21 12:00' and '2025-08-21 13:00'
and sigungu = '관악구'
        order by 1 desc,2
--   select 
--     e.slot_time,
